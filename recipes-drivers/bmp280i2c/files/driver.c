...
#define I2C_BUS_AVAILABLE   (2)              // I2C Bus available in Raspberry Pi
...
static int __init d_init(void)
{
  ...
  if((alloc_chrdev_region(&dev, 0, 1, "etx_Dev_new")) <0){
  ...
  if((dev_class = class_create(THIS_MODULE,"etx_class_new")) == NULL){
    pr_err("Cannot create the struct class\n");
    return -1;
  }

    /*Creating device*/
  if((device_create(dev_class,NULL,dev,NULL,"etx_device_new")) == NULL){
  ...
  if((cdev_add(&etx_cdev,dev,1)) < 0){
    ...
    if( etx_i2c_adapter != NULL )
    {
        ...
        if( etx_i2c_client_bmp != NULL )
        {
            ...
            // i2c_register_driver(&etx_bmp_driver);
            ret = 0;
        }
...
static void __exit etx_driver_exit(void)
{
    ...
    pr_info("1 Driver Removed!!!\n");

    i2c_unregister_device(etx_i2c_client_bmp);
    pr_info("2 Driver Removed!!!\n");



    device_destroy(dev_class,dev);
    pr_info("3 Driver Removed!!!\n");
    class_destroy(dev_class);
    pr_info("4 Driver Removed!!!\n");

    cdev_del(&etx_cdev);
    pr_info("5 Driver Removed!!!\n");

    unregister_chrdev_region(dev, 1);
    pr_info(" 6 Driver Removed!!!\n");

    pr_info(" 7 Driver Removed!!!\n");
}
